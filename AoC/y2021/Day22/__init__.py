...
@Task(year=2021, day=22, task=1)
def run_t1(data: List[_operation], log: Callable[[str], None]) -> Any:
    small_range: Tuple[int, int] = (-50, 50)
    ranges: List[Tuple[int, int]] = [small_range for _ in range(len(data[0]) - 1)]
    return _run(ranges=ranges, ops=data, sim_method=sim_reactor_f, log=log)


@Task(year=2021, day=22, task=2)
def run_t2(data: List[_operation], log: Callable[[str], None]) -> Any:
    return _run(ops=data, sim_method=sim_reactor_f, log=log)


def _run(
        ops: List[_operation], sim_method: Callable[[Optional[Tuple[Tuple[int, int], ...]],
                                                     List[_operation], Callable[[str], None]], Union[int, "Area"]],
        log: Callable[[str], None], ranges: Optional[Iterable[_coord_range]] = None
) -> Any:
    log(f"Using {len(ops)} potential operations")
    log("Restricting reactor ranges to")
    if ranges is None:
        log("Reactor activation sequence is in unrestricted mode. Every cube is available")
        log("UNLIMITED POWAAAAAAAAAAAAAAAAAAA")
    else:
        c = {0: "x", 1: "y", 2: "z"}
        for i, r in enumerate(ranges):
            log(f"  {c.get(i, '_')} from {r[0]} to {r[1]}")
    ...
    log(f"In the end {ret} cubes are on")
    log(f"  That is {percentage}% [{ret}/{full_area}]")
    log(f"  of the cubic area around the reactor")
    return ret


def sim_reactor(ranges: Optional[Iterable[_coord_range]], ops: List[_operation], log: Callable[[str], None]) -> "Area":
    ...
    for op, *bounds in tqdm.tqdm(ops, desc="Applying operations", leave=False, unit="o"):
        if ranges is not None:
            bounds = tuple(adjust_range(r=b, max_shape=r, offset=0) for b, r in zip(bounds, ranges))
...
def sim_reactor_f(ranges: Optional[Iterable[_coord_range]], ops: List[_operation], log: Callable[[str], None]) -> int:
    if ranges is not None:
        ranges = list(ranges)
    ...
    log(f"In the end {len(cuboid_signs.keys())} patches were registered and counted")
